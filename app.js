...
// ============ Быстрые кнопки количества (по 3 в ряд, .qty-preset-btn) ============
...
// ============ Видимость голосового ввода и комментариев ============
...
function loadVisibilityPrefsUI() {
  document.getElementById('enable-voice').checked = localStorage.getItem('showVoice') !== '' ? !!localStorage.getItem('showVoice') : true;
  document.getElementById('enable-comments').checked = localStorage.getItem('showComments') !== '' ? !!localStorage.getItem('showComments') : true;
}
...
function getPeriodDates(period) {
  ...
  switch (period) {
    ...
    case 'custom':
      ...
      break;
    default:
      dateFrom = new Date(0); dateTo = new Date();
  }
